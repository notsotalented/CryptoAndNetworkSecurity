<?php
	...
	if ($pass != $pass_r) {
		echo "<script>
			window.location.href='index.php?page=register';
			alert('The provided passwords did not match each another!');
		</script>";
	}
	...
	$sql = "select *from login_info where username = '$usernames' and password = '$pass' and role = '$role'";  
	...
	if ($count != 1) {
		echo "<script>
			window.location.href='index.php?page=terminate';
			alert('Username not found');
		</script>";
	}
	else {

		$sql_delete =  "delete from login_info where username = '$id'";
